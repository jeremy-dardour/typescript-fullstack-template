 ...
 * VSA/DDD style: modules cannot import each other, only shared and app
 */
function getModulesPreset(): {
  ...
  return {
    ...
    rules: [
      {
        // Modules can only import: same module, shared, app
        from: ['module'],
        allow: [
      ...
      {
        // shared can import itself only
        from: ['shared'],
        allow: ['shared'],
        message: 'shared cannot import other modules',
      },
      {
        // app can import shared, but not business modules
        from: ['app'],
        allow: ['app', 'shared'],
